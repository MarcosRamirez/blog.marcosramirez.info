...
En cuanto añadí un agente de voz a la web —del que hablo en detalle en el post [De VAPI a Retell: la migración que se llevó media arquitectura](/de-vapi-a-retell-migracion-agente-de-voz/){:target="_blank"}— necesité una ruta de API en el servidor para generar tokens de acceso de forma segura. Y ahí empezó la guerra.
