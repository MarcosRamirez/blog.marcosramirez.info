...
- Category archive pages are accessible at `/categories/<category-slug>/`
...
Jekyll does not have a `category_url` Liquid tag like `post_url`. To link to a category archive page, use the `slugify` filter on the category name to generate the correct slug:
```liquid
...
<a href="{{ "/categories/" | append: category | slugify | append: "/" | relative_url }}">{{ category }}</a>
{% endfor %}
```